...
app.get('/api', (req, res) => {
    ...
    const getActions = {
        ...
        'ds_get_pominiete': '/api/klienci?action=ds_get_pominiete&tenant_id=' + tenant_id,
        'ds_przeglad': '/api/klienci?action=ds_przeglad&tenant_id=' + tenant_id,
        ...
        'get_aktywne_pakiety': '/api/klienci?action=get_aktywne_pakiety&tenant_id=' + tenant_id + '&zabieg=' + encodeURIComponent(req.query.zabieg || ''),
        'get_pakiety_z_aktywnymi_lista': '/api/klienci?action=get_pakiety_z_aktywnymi_lista&tenant_id=' + tenant_id,
...
app.post('/api', (req, res) => {
    ...
    const sprzedazActions = ['add_sale', 'edit_sale', 'delete_sale', 'add_zwrot', 'add_sales_def', 'add_multi_sale', 'emergency_edit_sale', 'add_discount_def', 'delete_employee', 'delete_service', 'edit_service', 'extend_karnet', 'close_karnet', 'reopen_karnet', 'suspend_karnet', 'suspend_all_karnety'];
    const klienciActions = ['add_client', 'add_client_fast_sales', 'edit_client_data', 'save_client_memo', 'manage_deposit', 'merge_deposits', 'add_suggestion_rule', 'delete_suggestion_rule', 'soft_delete_client', 'anonymize_client', 'mark_deceased', 'set_warning', 'clear_warning', 'restore_client', 'mark_retail_proposed', 'unmark_retail_proposed', 'ds_pomin', 'ds_przywroc', 'ds_przypisz_zadatek', 'set_odwolanie_ulga', 'clear_odwolanie_ulga'];
    ...
    const analitykaActions = ['get_months', 'an_get_months', 'get_daily_summary', 'an_get_daily_summary', 'get_monthly_details', 'an_get_monthly_details', 'save_monthly_cost', 'an_save_monthly_cost', 'get_costs_list', 'an_get_costs_list', 'get_stats', 'an_get_stats', 'get_yearly_summary', 'an_get_yearly_summary', 'get_treatment_analysis', 'an_get_treatment_analysis', 'get_bi_data', 'an_get_bi_data', 'get_full_audit', 'an_get_full_audit'];
    const konsultacjeActions = ['kon_save_result', 'kon_update_result', 'kon_set_status', 'kon_add_consultant', 'kon_delete_consultant', 'kon_save_settings', 'kon_save_campaign', 'kon_toggle_campaign', 'akon_get_months', 'akon_get_stats', 'akon_get_daily_summary', 'akon_get_monthly_details', 'akon_get_monthly_list', 'odp_getReportData'];
    const targetyActions = ['add_target', 'get_targets', 'edit_target', 'tgt_get_employee_dashboard'];
    const raportActions = ['rap_getInventory', 'rap_getCategories', 'rap_getLogs', 'rap_updateStock', 'rap_archiveProduct', 'rap_saveProduct', 'rap_saveCategory', 'rap_deleteCategory', 'rap_getHidden', 'rap_hideReorder', 'rap_unhideReorder'];
...
db.query(`
    CREATE TABLE IF NOT EXISTS Koszty_Kategorie (
        ...
        INDEX idx_kkat_tenant (tenant_id),
        UNIQUE KEY uniq_kkat_tenant_lp (tenant_id, lp)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
`, (err) => { if (err) console.error('[migration] Koszty_Kategorie:', err.message); else console.log('[migration] Koszty_Kategorie OK'); });
...
db.query(`
    CREATE TABLE IF NOT EXISTS Koszty_Szczegoly (
        ...
        INDEX idx_kszcz_tenant_month (tenant_id, miesiac_rok),
        UNIQUE KEY uniq_kszcz_tenant_month_kat (tenant_id, miesiac_rok, kategoria_id),
        CONSTRAINT fk_kszcz_kat FOREIGN KEY (kategoria_id) REFERENCES Koszty_Kategorie(id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
`, (err) => { if (err) console.error('[migration] Koszty_Szczegoly:', err.message); else console.log('[migration] Koszty_Szczegoly OK'); });
