...
**Faza 3 — nagrody.** ✅ ZREALIZOWANE 2026-07-10 (dev). Tabele `Lojalnosc_Nagrody` + `Lojalnosc_Odbiory`. Apka: katalog nagród (zdjęcie-URL/emoji, koszt, „za X pkt" gdy nie stać), pasek postępu do najbliższej nagrody, Odbierz → **rezerwacja punktów** (saldo dostępne = saldo − OCZEKUJE, limit 3 oczekujących, limit sztuk nagrody) → 6-znakowy kod (bez mylnych znaków) → klient może anulować. Panel (sekcja Klub → Odbiory): Wydaj (dopiero WTEDY wpis -pkt w ledger, zrodlo NAGRODA, idempotentnie ref ODB@id) / Odrzuć (rezerwacja znika).

**Faza 4 — promocje + push.** ✅ ZREALIZOWANE 2026-07-10 (dev). Tabele `Lojalnosc_Promocje` + `Lojalnosc_Zgloszenia` + `Lojalnosc_Push`. Apka: banery (zdjęcie, okno dat), promocja z przyciskiem BIORĘ (zgłoszenie do salonu, dedupe per klient+promocja), włączanie powiadomień w Profilu. Push: **web-push (VAPID)** — klucze w env (`VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`, lokalnie już w .env, NA HOSTINGERZE TRZEBA DODAĆ — bez apostrofów!); brak kluczy = push wyłączony gracefully; wysyłka z panelu (zakładka Promocje, też przycisk PUSH przy promocji), martwe subskrypcje 404/410 sprzątane. sw.js: push + notificationclick. **UWAGA deploy: doszła zależność `web-push` → pull + `npm install` + restart.**

**Faza 4b — KAMPANIE (targetowanie + harmonogram).** ✅ ZREALIZOWANE 2026-07-10 (wieczór). Segmenty: wszyscy / saldo ≥ X pkt / po zabiegu (typ lub nazwa, okno dni — z PARAGONÓW, nie z parsowania Booksy jak w Gemini) / bez wizyty od N dni (odzyskiwanie klientek). Kampania = push + wiadomość w apce przez 30 dni (kanał gwarantowany, działa bez VAPID). Harmonogram: „wyślij teraz" albo data+godzina (scheduler co 60 s, atomowy claim — wspólna baza dev+prod). Promocje: targetowane per segment (klient spoza segmentu ich nie widzi) + checkbox auto-push w dniu startu. Panel: zakładka 📬 Kampanie. Apka: sekcja „Wiadomości od salonu".

**Faza 5 — produktyzacja.** CZĘŚCIOWO 2026-07-10: ✅ sekcja „💎 Klub" w panelu (menu Moje Estelio+, admin-only przez flagę adminOnly w FEATURE_MENU_CONFIG) z zakładkami Ustawienia (nazwa klubu, pkt/10 zł) / Nagrody (CRUD) / Promocje (CRUD + push) / Odbiory i zgłoszenia (badge liczby) / Statystyki (KPI m-c, top klienci, wydane nagrody). POZOSTAJE: cena dodatku + odkrycie w katalogu, upload zdjęć nagród (dziś tylko URL), ikona PNG dla iOS, landing/blog.
