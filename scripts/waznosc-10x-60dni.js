// scripts/waznosc-10x-60dni.js
// Masowa zmiana Uslugi.waznosc_dni -> 60 dla zabiegow z wariantem zawierajacym '10x'.
//   node scripts/waznosc-10x-60dni.js                 # DRY-RUN (tylko podglad)
//   node scripts/waznosc-10x-60dni.js --wet-run       # wlasciwy UPDATE
//   node scripts/waznosc-10x-60dni.js --tenant=ID     # ogranicz do jednego salonu
require('dotenv').config();
const mysql = require('mysql2');

const WET_RUN = process.argv.includes('--wet-run');
const tenantArg = (process.argv.find(a => a.startsWith('--tenant=')) || '').split('=')[1] || null;
const NOWA_WAZNOSC = 60;
...
async function main() {
  ...
  console.log('Filtr:  ', "LOWER(wariant) LIKE '%10x%'" + (tenantArg ? ` AND tenant_id = '${tenantArg}'` : ' (WSZYSTKIE salony)'));
  console.log('');

  const where = "LOWER(wariant) LIKE '%10x%'" + (tenantArg ? ' AND tenant_id = ?' : '');
