// scripts/waznosc-5x-30dni.js
// Masowa zmiana Uslugi.waznosc_dni -> 30 dla zabiegow z wariantem zawierajacym '5x'.
//   node scripts/waznosc-5x-30dni.js                 # DRY-RUN (tylko podglad)
//   node scripts/waznosc-5x-30dni.js --wet-run       # wlasciwy UPDATE
//   node scripts/waznosc-5x-30dni.js --tenant=ID     # ogranicz do jednego salonu
require('dotenv').config();
const mysql = require('mysql2');

const WET_RUN = process.argv.includes('--wet-run');
const tenantArg = (process.argv.find(a => a.startsWith('--tenant=')) || '').split('=')[1] || null;
const NOWA_WAZNOSC = 30;
...
async function main() {
  ...
  console.log('Filtr:  ', "LOWER(wariant) LIKE '%5x%' AND NOT LIKE '%15x%'" + (tenantArg ? ` AND tenant_id = '${tenantArg}'` : ' (WSZYSTKIE salony)'));
  console.log('');

  const where = "LOWER(wariant) LIKE '%5x%' AND LOWER(wariant) NOT LIKE '%15x%'" + (tenantArg ? ' AND tenant_id = ?' : '');
