// scripts/waznosc-4x-30dni.js
// Masowa zmiana Uslugi.waznosc_dni -> 30 dla zabiegow z wariantem zawierajacym '4x'.
//   node scripts/waznosc-4x-30dni.js                 # DRY-RUN (tylko podglad)
//   node scripts/waznosc-4x-30dni.js --wet-run       # wlasciwy UPDATE
//   node scripts/waznosc-4x-30dni.js --tenant=ID     # ogranicz do jednego salonu
...
async function main() {
  ...
  console.log('Filtr:  ', "LOWER(wariant) LIKE '%4x%'" + (tenantArg ? ` AND tenant_id = '${tenantArg}'` : ' (WSZYSTKIE salony)'));
  console.log('');

  const where = "LOWER(wariant) LIKE '%4x%'" + (tenantArg ? ' AND tenant_id = ?' : '');
