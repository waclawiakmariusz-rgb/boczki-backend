...
async function wyslijLinkRejestracji({ email, imie, token, nazwa_salonu }) {
  ...
  await transport.sendMail({
    ...
    html: emailWrapper('✨', 'Estelio', 'System zarządzania salonem', `
      <p style="font-size:16px; font-weight:700; color:#1e1b4b; margin-bottom:8px;">Cześć${imie ? ' ' + imie : ''}! 👋</p>
      <p style="font-size:14px; color:#64748b; line-height:1.7; margin-bottom:24px;">
        Dziękujemy za wybór systemu <strong>Estelio</strong>.
...
async function powiadomAdmina({ imie, nazwa_salonu, email, telefon, miasto, wiadomosc }) {
  ...
  await transport.sendMail({
    ...
    html: emailWrapper('📋', 'Estelio', 'Panel administratora', `
      <h2 style="font-size:16px; color:#1e1b4b; margin-top:0;">Nowe zgłoszenie rejestracji</h2>
      <table style="width:100%; border-collapse:collapse; font-size:14px;">
        <tr><td style="padding:7px 0; color:#64748b; width:140px;">Imię / kontakt:</td><td style="padding:7px 0; font-weight:600; color:#1e293b;">${imie}</td></tr>
        <tr><td style="padding:7px 0; color:#64748b;">Nazwa salonu:</td><td style="padding:7px 0; font-weight:600; color:#1e293b;">${nazwa_salonu}</td></tr>
        ...
        ${telefon ? `<tr><td style="padding:7px 0; color:#64748b;">Telefon:</td><td style="padding:7px 0; color:#1e293b;">${telefon}</td></tr>` : ''}
...
async function wyslijResetHasla({ email, login, token }) {
  ...
  await transport.sendMail({
    ...
    html: emailWrapper('🔑', 'Estelio', 'Reset hasła', `
      <p style="font-size:15px; color:#1e1b4b; margin-bottom:8px;">Cześć <strong>${login}</strong> 👋</p>
      <p style="font-size:14px; color:#64748b; line-height:1.7; margin-bottom:24px;">
